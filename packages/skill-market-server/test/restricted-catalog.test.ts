...
describe("restricted catalog", () => {
  test("uses one current authorization policy for summaries, detail, and versions", async () => {
    ...
    try {
      ...
      await expect(fixture.catalog.require(fixture.principals.outsider, "pub_groupmember")).rejects.toBeInstanceOf(
        SkillMarketSecurityError,
      )
      await expect(fixture.catalog.require(fixture.principals.group, "pub_disabledgrp")).rejects.toBeInstanceOf(
        SkillMarketSecurityError,
      )
      await expect(fixture.catalog.require(fixture.principals.owner, "pub_delisted001")).rejects.toBeInstanceOf(
        SkillMarketSecurityError,
      )
      await expect(fixture.catalog.require(fixture.principals.owner, "pub_missing0001")).rejects.toBeInstanceOf(
        SkillMarketSecurityError,
      )
  ...
  test("issues only hashed exact ten-minute grants and rechecks access until expiry", async () => {
    const fixture = await restrictedFixture()
    try {
      const grant = await fixture.grants.issue(fixture.principals.group, "pub_groupmember")
      ...
      expect(await fixture.grants.resolve(token)).toMatchObject({
        publicationID: "pub_groupmember",
        employeeID: "group",
        key: "skill-market-private/group.zip",
      })
      expect(await fixture.grants.resolve("malformed")).toBeUndefined()
      ...
      expect(await fixture.grants.resolve(token)).toBeUndefined()
      await fixture.database.transaction(async (c) =>
        c.run(
...
async function restrictedFixture() {
  ...
  await database.transaction(async (connection) => {
    ...
    for (const [employeeID, display, departmentID] of [
      ...
      await connection.run(
        ...
        [employeeID, display, departmentID, initialNow, initialNow],
      )
    await connection.run(
    ...
    await seedPublication(connection, {
      id: "pub_department1",
