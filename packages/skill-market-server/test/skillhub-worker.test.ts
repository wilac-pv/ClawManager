...
test("runs bounded discovery and only publishes when its due callback permits it", async () => {
  ...
  const result = await runSkillHubWorker({
    ...
    progress: async () => ({ mirrored: 1 }),
    publicationCheckpoint: async () => ({ lastPublishedCount: 0 }),
    shouldPublish: () => true,
    emit: () => undefined,
    publish: async () => {
      calls.push("publish")
...
test("does not record a checkpoint when an active catalog publication defers the publish", async () => {
  ...
  const result = await runSkillHubWorker({
    ...
    discover: async () => undefined,
    mirror: { runBatch: async () => ({ mirrored: 0, retryWait: 0, rejected: 0 }) },
    progress: async () => ({ mirrored: 80_513 }),
    publicationCheckpoint: async () => ({ lastPublishedCount: 80_226 }),
    shouldPublish: () => true,
    publish: async () => false,
