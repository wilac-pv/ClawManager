...
describe("personal Skill trash", () => {
  test("soft deletes only the owner's published personal Skill and restores strictly before seven days", async () => {
    ...
    const personal = seedPersonal(fixture, "sub_personaltrash1", "private/personal/package.zip")
    ...
    expectCode(() => trash.deletePersonal(fixture.bob, personal.id, personal.version), "not-found")
    expectCode(() => trash.deletePersonal(fixture.alice, personal.id, personal.version + 1), "submission-conflict")

    const deleted = trash.deletePersonal(fixture.alice, personal.id, personal.version)
    ...
    expect(trash.list(fixture.alice).map((item) => item.id)).toEqual([personal.id])
    expect(trash.list(fixture.bob)).toEqual([])
    expect(submissions.listOwn(fixture.alice, { target: "personal", page: 1, limit: 30 }).items).toEqual([])
    expect(() => submissions.getOwn(fixture.alice, personal.id)).toThrow("not found")
    expect(() => submissions.personalPackage(fixture.alice, personal.id)).toThrow("not found")
    expect(() =>
    ...
    expect(trash.restorePersonal(fixture.alice, personal.id, deleted.version)).toMatchObject({
      id: personal.id,
      version: deleted.version + 1,
    })

    const deletedAgain = trash.deletePersonal(fixture.alice, personal.id, deleted.version + 1)
    fixture.clock.value += week
    expectCode(() => trash.restorePersonal(fixture.alice, personal.id, deletedAgain.version), "not-found")
    const audits = fixture.database.connection
      .query<{ action: string; before_json: string | null; after_json: string | null }, []>(
  ...
  test("purges expired unreferenced artifacts and durably skips them on retry", async () => {
    const fixture = await trashFixture()
    const personal = seedPersonal(fixture, "sub_personalpurge1", "private/purge/package.zip", true)
    ...
    expect(
      fixture.database.connection
        .query<{ artifacts_purged_at: number | null }, [string]>(
          "SELECT artifacts_purged_at FROM submissions WHERE id = ?",
        )
        .get(personal.id)?.artifacts_purged_at,
  ...
  test("preserves revision and restricted publication objects and retries an object deletion failure", async () => {
    const fixture = await trashFixture()
    const shared = seedPersonal(fixture, "sub_personalshared1", "private/shared/package.zip")
    seedRevisionReference(fixture, "sub_restrictedref1", shared.packageKey)
    const published = seedPersonal(fixture, "sub_personalshared2", "private/published/package.zip")
    ...
    const trash = createPersonalTrash({ database: fixture.database, store, now: () => fixture.clock.value })
    ;[shared, published, partial].forEach((item) => trash.deletePersonal(fixture.alice, item.id, item.version))
    fixture.clock.value += week
    ...
    expect(
      fixture.database.connection
        ...
        .get(partial.id)?.artifacts_purged_at,
    ).toBeNull()
...
function seedRevisionReference(
  fixture: Awaited<ReturnType<typeof trashFixture>>,
  id: SkillMarketControl.SubmissionID,
  packageKey: string,
) {
...
function seedRestrictedReference(
  fixture: Awaited<ReturnType<typeof trashFixture>>,
  id: SkillMarketControl.SubmissionID,
  packageKey: string,
) {
  seedRevisionReference(fixture, id, `private/reference/${id}/package.zip`)
  fixture.database.connection.run(
...
function expectCode(operation: () => unknown, code: SkillMarketControl.ProblemCode) {
  try {
    operation()
  } catch (error) {
