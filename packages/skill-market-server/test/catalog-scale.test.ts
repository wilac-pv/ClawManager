...
test("samples catalog RSS after serializing the representative V2 payload", async () => {
  const fixture = await Bun.file(new URL("./catalog-scale-fixture.ts", import.meta.url)).text()

  expect(fixture.indexOf("const catalogPayload = JSON.stringify(")).toBeGreaterThanOrEqual(0)
  expect(fixture.indexOf("const maxRSS = process.resourceUsage().maxRSS")).toBeGreaterThan(
    fixture.indexOf("const catalogPayload = JSON.stringify("),
  )
})
