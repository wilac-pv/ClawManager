...
test("sweeps typography across the supported desktop and mobile pages", async ({ page }) => {
  for (const viewport of [
    ...
    const hash = page.locator(".ruying-skill-market__versions code").first()
    await expect(hash).toHaveText("a".repeat(64))
    await expect(hash).toHaveJSProperty("scrollWidth", await hash.evaluate((element) => element.clientWidth))

    await page.route(`${fixtureApi}/v1/favorites`, (route) => route.abort())
    await page.goto("/favorites")
    await expect(page.getByRole("alert").getByRole("heading", { name: "收藏加载失败", level: 2 })).toBeVisible()
    await page.unroute(`${fixtureApi}/v1/favorites`)
