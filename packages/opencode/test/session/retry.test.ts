...
describe("session.retry.retryable", () => {
  ...
  test("does not emit a public upsell for free limits", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    expect(SessionRetry.retryable(error, "ruying")).toEqual({ message: "Free usage exceeded" })
  })

  test("does not emit a public workspace link for subscription limits", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    expect(SessionRetry.retryable(error, "ruying")).toEqual({
      message: "Subscription quota exceeded. You can continue using free models.",
    })
  })

  test("keeps subscription errors generic without limit metadata", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
        message: "Subscription quota exceeded. You can continue using free models.",
        isRetryable: true,
