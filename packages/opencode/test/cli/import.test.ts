...
test("rejects only remote and Windows network paths before filesystem service access", async () => {
  ...
  expect(isNetworkImportPath("\\\\?\\C:/sessions\\session.json", "linux")).toBe(true)
  ...
  expect(isNetworkImportPath("\\\\?\\C:\\sessions\\session.json", "win32")).toBe(false)
  expect(isNetworkImportPath("//?/C:/sessions/session.json", "win32")).toBe(false)
  expect(isNetworkImportPath(`\\\\?\\${volume}\\sessions\\session.json`, "win32")).toBe(false)
  expect(isNetworkImportPath(`//?/${volume}/sessions/session.json`, "win32")).toBe(false)
  expect(isNetworkImportPath("\\\\.\\C:\\sessions\\session.json", "win32")).toBe(true)
  expect(isNetworkImportPath("\\\\.\\pipe\\ruying", "win32")).toBe(true)
  expect(isNetworkImportPath("\\\\?\\GLOBALROOT\\Device\\HarddiskVolume1\\session.json", "win32")).toBe(true)
  expect(isNetworkImportPath("\\\\?\\Device\\Mup\\server\\share\\session.json", "win32")).toBe(true)
  expect(isNetworkImportPath("\\\\;\\LanmanRedirector\\server\\share\\session.json", "win32")).toBe(true)
  ...
  expect(requireLocalImportPath("C://sessions/session.json")).toBe("C://sessions/session.json")
  expect(() => requireLocalImportPath("\\\\?\\C:\\sessions\\session.json")).toThrow("local filesystem")
})
