...
import { platformDirectory, platformManifest, releaseArtifact, SUPPORTED_TARGETS } from "./publish-ruying"
...
if (Script.release) {
  for (const item of targets) {
    ...
    if (item.os === "linux") {
      await $`tar -czf ../../${artifact.archive} *`.cwd(`dist/${artifact.directory}/bin`)
    } else {
      await $`zip -r ../../${artifact.archive} *`.cwd(`dist/${artifact.directory}/bin`)
    }
  }
