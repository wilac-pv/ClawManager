    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (
            ConfigPaths.projectDirectoryNames.some((name) => dir.endsWith(name)) ||
          ...
          // Auto-discovered plugins under project config directories are already local files, so ConfigPlugin.load
