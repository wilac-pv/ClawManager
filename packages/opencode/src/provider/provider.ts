  ...
  return {
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Title": Brand.profile.englishName,
            "X-Source": Brand.profile.cliName,
          },
        },
      }),
    openrouter: () =>
    ...
    nvidia: (provider) =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Title": Brand.profile.englishName,
            "X-BILLING-INVOKE-ORIGIN": Brand.profile.englishName,
          },
        },
      }),
    vercel: () =>
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `${Brand.profile.cliName}/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `${Brand.profile.cliName}/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            `Set it via environment variable or run \`${Brand.profile.cliName} auth cloudflare-ai-gateway\`.`,
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `${Brand.profile.cliName}/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": Brand.profile.cliName,
          },
        },
      }),
    kilo: () =>
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, ${Brand.profile.cliName} auth, or provider options.`,
            )
          },
        }
      }
