...
const main = Effect.gen(function* () {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("ruying-code://") || arg.startsWith("opencode://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("ruying-code")
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const { listener, health } = yield* Effect.promise(() =>
      spawnLocalServer(hostname, port, password, {
        ...
        legacyUserDataPath: legacyElectronDataPath(app.getPath("appData"), app.isPackaged ? CHANNEL : "dev"),
