...
import { type WslCommandLine, resolveWslRuyingCode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const ruyingCode = await resolveWslRuyingCode(distro)
  if (!ruyingCode) throw new Error(`Ruying Code is not installed in ${distro}`)
  ...
  const script = [
    ...
    `exec ${shellEscape(ruyingCode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
