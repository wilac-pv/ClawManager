  ...
  return {
    ...
    async installOpencode(name: string) {
      await runJob({ kind: "install-opencode", distro: name, startedAt: Date.now() }, async (abort) => {
        ...
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Ruying Code installation failed")
        }
...
function opencodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "Ruying Code is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "Ruying Code is installed but could not run",
    }
  }
  return {
    distro,
