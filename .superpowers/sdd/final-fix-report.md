...
- Reworked WSL PATH handling to preserve inherited Linux entries such as nvm, Volta, asdf, `/opt`, and snap while filtering every `/mnt` entry. The sanitizer invokes system tools by absolute Linux paths so a hostile or Windows-only inherited PATH cannot replace the filtering utilities. Mounted resolved executables remain rejected.
- Branded the active `/init` template and all visible updater/settings locale values as Ruying Code, removed the unsupported `$schema` TUI tip, and added behavioral callback/import/User-Agent plus all-locale audits.

## Wave 6 RED evidence

- Full credential audit found one tracked design-document match and four ignored stale package matches.
