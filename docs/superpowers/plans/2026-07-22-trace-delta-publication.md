...
- Production service limits are `MemoryHigh=896M`, `MemoryMax=1024M`, `TimeoutStartSec=90s`, and `TimeoutStopSec=10s`.
...
- Produces: a release unit bounded by `MemoryHigh=896M`, `MemoryMax=1024M`, `TimeoutStartSec=90s`, and `TimeoutStopSec=10s`.
...
expect(service).toContain("TimeoutStartSec=90s")
